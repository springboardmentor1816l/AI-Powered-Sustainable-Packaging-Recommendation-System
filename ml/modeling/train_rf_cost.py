"""
EcoPackAI - Random Forest Cost Prediction Model Training
Trains an advanced Random Forest model for packaging cost prediction
...
print("="*80)
print(" " * 15 + "ECOPACKAI - RANDOM FOREST COST PREDICTION")
print("="*80)
print("\nTraining advanced model to improve upon baseline performance...")
...
def load_and_prepare_data():
    """Load integrated dataset and prepare features for cost prediction"""
    print("\n" + "="*80)
    ...
    exclude_cols = [
        ...
        'cost_per_unit_usd',  # Target variable
        'co2_emission_per_kg_estimated'  # Other target (not for cost model)
    ]
    ...
    print(f"  - Target units: USD per unit")
    ...
    for cat in categorical_cols[:10]:  # Show first 10
        print(f"      • {cat}")
    if len(categorical_cols) > 10:
        print(f"      ... and {len(categorical_cols) - 10} more")
    
    print(f"  - Numeric features: {len(numeric_cols)}")
    for num in numeric_cols[:10]:  # Show first 10
        print(f"      • {num}")
    ...
    print(f"  ✓ No derived cost features in X")
...
def configure_random_forest():
    """Configure Random Forest model with optimized hyperparameters"""
    print("\n" + "="*80)
    print("STEP 3: CONFIGURING RANDOM FOREST MODEL")
    print("="*80)
    ...
    print(f"  - Random seed: {rf_params['random_state']}")
    print(f"  - Parallel jobs: All CPU cores")
    
    print(f"\n📋 Model Rationale:")
    print(f"  - Ensemble method combines multiple decision trees")
...
def compare_with_baseline(rf_metrics):
    """Compare Random Forest with baseline Linear Regression"""
    print("\n" + "="*80)
    ...
    baseline_r2 = 0.9952
    baseline_mae = 0.3702
    baseline_rmse = 0.4914
    
    rf_r2 = rf_metrics['test_r2']
    rf_mae = rf_metrics['test_mae']
    rf_rmse = rf_metrics['test_rmse']
    
    print(f"\n📊 Performance Comparison:")
    print(f"\n  {'Metric':<15} {'Baseline (LR)':<20} {'Random Forest':<20} {'Improvement'}")
    print(f"  {'-'*75}")
    
    # R² comparison
    r2_diff = rf_r2 - baseline_r2
    r2_pct = (r2_diff / baseline_r2) * 100 if baseline_r2 != 0 else 0
    r2_symbol = "✅" if r2_diff > 0 else "➖" if r2_diff == 0 else "⚠️"
    print(f"  {'R²':<15} {baseline_r2:<20.6f} {rf_r2:<20.6f} {r2_symbol} {r2_diff:+.6f} ({r2_pct:+.2f}%)")
    
    # MAE comparison (lower is better)
    mae_diff = baseline_mae - rf_mae  # Positive means RF is better
    mae_pct = (mae_diff / baseline_mae) * 100 if baseline_mae != 0 else 0
    mae_symbol = "✅" if mae_diff > 0 else "➖" if mae_diff == 0 else "⚠️"
    print(f"  {'MAE':<15} ${baseline_mae:<19.4f} ${rf_mae:<19.4f} {mae_symbol} {mae_diff:+.4f} ({mae_pct:+.2f}%)")
    
    # RMSE comparison (lower is better)
    rmse_diff = baseline_rmse - rf_rmse  # Positive means RF is better
    rmse_pct = (rmse_diff / baseline_rmse) * 100 if baseline_rmse != 0 else 0
    rmse_symbol = "✅" if rmse_diff > 0 else "➖" if rmse_diff == 0 else "⚠️"
    print(f"  {'RMSE':<15} ${baseline_rmse:<19.4f} ${rf_rmse:<19.4f} {rmse_symbol} {rmse_diff:+.4f} ({rmse_pct:+.2f}%)")
    
    print(f"\n  Legend: ✅ Improvement | ➖ No change | ⚠️ Degradation")
    
    # Overall assessment
    improvements = sum([r2_diff > 0, mae_diff > 0, rmse_diff > 0])
    ...
    if improvements >= 2:
        print(f"  ✅ Random Forest shows improvement over baseline ({improvements}/3 metrics)")
    elif improvements == 1:
        print(f"  ➖ Random Forest shows mixed results ({improvements}/3 metrics improved)")
    else:
        print(f"  ⚠️  Baseline performs similarly or better")
...
def save_metrics(metrics, cv_metrics, comparison):
    ...
    # Save to CSV
    output_path = METRICS_DIR / 'rf_cost_metrics.csv'
    metrics_df.to_csv(output_path, index=False)
    
    print(f"  ✓ Metrics saved to: {output_path}")


def generate_training_summary(rf_params, metrics, cv_metrics, comparison, feature_cols):
    """Generate training summary documentation"""
    print("\n📝 Generating training summary...")
    
    doc_path = DOCS_DIR / 'rf_cost_training_summary.md'
    
    with open(doc_path, 'w', encoding='utf-8') as f:
        f.write("# Random Forest Cost Prediction - Training Summary\n\n")
        f.write("## Overview\n\n")
        f.write("This document summarizes the Random Forest model trained for packaging cost prediction.\n\n")
        f.write(f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**Model Type:** Random Forest Regressor\n")
        f.write(f"**Target:** cost_per_unit_usd (USD per unit)\n\n")
        
        f.write("---\n\n")
        f.write("## Model Configuration\n\n")
        f.write("### Hyperparameters\n\n")
        f.write(f"- **Number of Trees:** {rf_params['n_estimators']}\n")
        f.write(f"- **Max Depth:** {rf_params['max_depth']}\n")
        f.write(f"- **Min Samples Split:** {rf_params['min_samples_split']}\n")
        f.write(f"- **Min Samples Leaf:** {rf_params['min_samples_leaf']}\n")
        f.write(f"- **Max Features:** {rf_params['max_features']}\n")
        f.write(f"- **Random Seed:** {rf_params['random_state']}\n\n")
        ...
        f.write(f"- **CV MAE:** ${cv_metrics['cv_test_mae_mean']:.4f} ± ${cv_metrics['cv_test_mae_std']:.4f}\n")
        f.write(f"- **CV RMSE:** ${cv_metrics['cv_test_rmse_mean']:.4f} ± ${cv_metrics['cv_test_rmse_std']:.4f}\n")
        f.write(f"- **CV R²:** {cv_metrics['cv_test_r2_mean']:.6f} ± {cv_metrics['cv_test_r2_std']:.6f}\n\n")
        
        f.write("---\n\n")
        f.write("## Comparison with Baseline\n\n")
        f.write("| Metric | Baseline (Linear Regression) | Random Forest | Improvement |\n")
        f.write("|--------|------------------------------|---------------|-------------|\n")
        f.write(f"| R² | {comparison['baseline_r2']:.6f} | {comparison['rf_r2']:.6f} | ")
        f.write(f"{comparison['r2_improvement']:+.6f} ({comparison['r2_improvement_pct']:+.2f}%) |\n")
        f.write(f"| MAE | ${comparison['baseline_mae']:.4f} | ${comparison['rf_mae']:.4f} | ")
        f.write(f"${comparison['mae_improvement']:+.4f} ({comparison['mae_improvement_pct']:+.2f}%) |\n")
        f.write(f"| RMSE | ${comparison['baseline_rmse']:.4f} | ${comparison['rf_rmse']:.4f} | ")
        f.write(f"${comparison['rmse_improvement']:+.4f} ({comparison['rmse_improvement_pct']:+.2f}%) |\n\n")
        
        f.write("### Assessment\n\n")
        if comparison['metrics_improved'] >= 2:
            f.write(f"✅ **Random Forest improves upon baseline** ({comparison['metrics_improved']}/3 metrics)\n\n")
        else:
        ...
        f.write(f"- **Model File:** `ml/models/rf_cost.joblib`\n")
        f.write(f"- **Metrics File:** `ml/metrics/rf_cost_metrics.csv`\n")
        ...
        f.write("# Load model\n")
        f.write("model = joblib.load('ml/models/rf_cost.joblib')\n\n")
        ...
        f.write("- ✅ Random Forest model trained successfully\n")
        f.write("- ✅ Cost prediction metrics computed and reviewed\n")
        f.write("- ✅ Model performance documented\n")
        f.write("- ✅ Model artifact saved correctly\n")
        f.write("- ✅ Training details documented\n\n")
        
        f.write("---\n\n")
        f.write("## Next Steps\n\n")
        f.write("1. Analyze feature importance\n")
        f.write("2. Perform error analysis on predictions\n")
        f.write("3. Consider hyperparameter tuning if needed\n")
        f.write("4. Deploy model to backend inference pipeline\n")
...
def main():
    ...
    rf_model, rf_params = configure_random_forest()
    
    # Step 4: Train model
    trained_model = train_random_forest(rf_model, X_train, y_train)
    ...
    # Step 7: Save model and results
    model_path = MODELS_DIR / 'rf_cost.joblib'
    save_model(trained_model, model_path)
    save_metrics(metrics, cv_metrics, comparison)
    generate_training_summary(rf_params, metrics, cv_metrics, comparison, feature_cols)
    ...
    print(f"  2. Metrics CSV: {METRICS_DIR / 'rf_cost_metrics.csv'}")
    print(f"  3. Training Summary: {DOCS_DIR / 'rf_cost_training_summary.md'}")
